...
1. **Dimensionamiento Técnico & Solar**:
   ...
   - **Ingeniería Transparente, Desglose Visual & Modo Clásico (Legacy Toggle)**: Separación explícita de Autoconsumo en Sitio (100% de tarifa), Inyección a Red (bruta y neta acreditada al 75%) y Ahorro Facturable total en Simulador, PDF y Propuesta Web Cloudflare. En el sidebar se visualiza una barra de distribución proporcional segmentada (Solar Directo Diurno, BESS Nocturno y Red Eléctrica) con métricas en kWh/día y porcentajes limpios sin textos redundantes. Incorpora un selector conmutador interactivo (`showSelfConsumptionBreakdown` / `showSelfConsumptionInProposal`) para alternar fácilmente entre la vista transparente detallada (7 columnas, curva azul) y el modo clásico tradicional (5 columnas: Mes, Consumo, Producción, Ahorro Facturable, % Cobertura, y 2 barras en gráficos) en el Simulador y en las Propuestas PDF.
...
# Suite integral de 15 pruebas unitarias financieras (incluyendo ítems extra, ITBIS y descuentos comerciales)
