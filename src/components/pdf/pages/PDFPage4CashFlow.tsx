...
export const PDFPage4CashFlow: React.FC<PDFPage4CashFlowProps> = ({
  ...
  return (
    <div className="pdf-page w-[850px] h-[1202px] min-h-[1202px] max-h-[1202px] bg-white shadow-xl flex flex-col shrink-0 relative overflow-hidden font-sans print:shadow-none print:w-full print:min-h-screen">
      ...
      {/* Body */}
      <div className="px-10 pt-2.5 pb-14 flex-1 flex flex-col justify-between gap-2.5 relative z-10 min-h-0">
        ...
        <div className="border border-slate-200 rounded-xl overflow-hidden shadow-xs text-xs flex-1 flex flex-col">
          <table className="w-full text-left border-collapse flex-1">
            <thead className="text-white font-bold uppercase tracking-wider text-[9.5px] shrink-0" style={{ backgroundColor: activeTheme.primary }}>
              <tr>
                <th className="px-2 py-1.5 w-10 text-center">Año</th>
                <th className="px-2 py-1.5 text-right">Energía Generada (kWh)</th>
                <th className="px-2 py-1.5 text-right">Ahorro Energía (USD)</th>
                <th className="px-2 py-1.5 text-right">Incentivo Fiscal (USD)</th>
                <th className="px-2 py-1.5 text-right">Ahorro Anual Total (USD)</th>
                <th className="px-2 py-1.5 text-right">Cash Flow (USD)</th>
                <th className="px-2 py-1.5 text-right font-bold">CF Beneficio Acumulado (USD)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 text-slate-700 font-semibold text-[10px]">
              ...
              {(() => {
                ...
                return (
                  <tr className="bg-red-50/70 text-red-700 font-bold">
                    ...
                    <td className="px-2 py-1 text-right text-red-600 font-extrabold">
                      -${initialOutflow.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </td>
                  </tr>
                );
              ...
              {cf25.map((row) => {
                ...
                return (
                  ...
                  >
                    <td className="px-2 py-[2.5px] text-center font-bold">{row.year}</td>
                    <td className="px-2 py-[2.5px] text-right font-medium">{row.productionKWh.toLocaleString()}</td>
                    <td className="px-2 py-[2.5px] text-right font-medium">
                      ${row.savingsUSD.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </td>
                    <td className="px-2 py-[2.5px] text-right font-semibold" style={{ color: activeTheme.secondary }}>
                      {row.taxCreditUSD > 0 ? `$${row.taxCreditUSD.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '$0.00'}
                    </td>
                    <td className="px-2 py-[2.5px] text-right font-semibold text-slate-900">
                      ${totalAnnualSavings.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </td>
                    <td className="px-2 py-[2.5px] text-right font-semibold" style={{ color: activeTheme.primary }}>
                      ${row.netCashFlowUSD.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </td>
                    <td
                      className="px-2 py-[2.5px] text-right font-bold"
        ...
        <div className="bg-slate-50/80 border border-slate-200 rounded-xl p-3 shrink-0">
          <h3 className="text-slate-800 font-bold text-xs mb-1.5 border-b border-slate-200 pb-1 flex items-center gap-2">
            <ShieldCheck className="w-4 h-4" style={{ color: activeTheme.primary }} /> Indicadores Financieros del Proyecto
          </h3>
          <div className="grid grid-cols-4 gap-y-1.5 gap-x-4 text-xs">
            <div>
              <p className="text-slate-400 text-[9.5px] uppercase tracking-wider font-bold mb-0.5">Payback</p>
              <p className="font-bold text-slate-900">{summary.paybackYears} años</p>
            </div>
            <div>
              <p className="text-slate-400 text-[9.5px] uppercase tracking-wider font-bold mb-0.5">TIR</p>
              <p className="font-bold" style={{ color: activeTheme.primary }}>{summary.irrPct}%</p>
            </div>
            <div>
              <p className="text-slate-400 text-[9.5px] uppercase tracking-wider font-bold mb-0.5">VAN (10%)</p>
            ...
            <div>
              <p className="text-slate-400 text-[9.5px] uppercase tracking-wider font-bold mb-0.5">Ahorro Total 25 Años</p>
            ...
            <div>
              <p className="text-slate-400 text-[9.5px] uppercase tracking-wider font-bold mb-0.5">ROI Total</p>
              <p className="font-bold" style={{ color: activeTheme.primary }}>{summary.roi25YrPct}%</p>
            </div>
            <div>
              <p className="text-slate-400 text-[9.5px] uppercase tracking-wider font-bold mb-0.5">Reducción CO2</p>
              <p className="font-bold text-slate-900">{(summary.co2AvoidedTonsPerYear * 25).toFixed(1)} Ton</p>
            </div>
            <div>
              <p className="text-slate-400 text-[9.5px] uppercase tracking-wider font-bold mb-0.5">Precio por Watt</p>
              <p className="font-bold text-slate-900">${(project.specs.pricePerWattUSD || project.financials.pricePerWattUSD).toFixed(3)} USD/W</p>
            </div>
            <div>
              <p className="text-slate-400 text-[9.5px] uppercase tracking-wider font-bold mb-0.5">Capacidad DC</p>
