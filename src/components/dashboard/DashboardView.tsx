...
export const DashboardView: React.FC = () => {
  ...
  return (
    <div
      className={`flex-1 overflow-y-auto p-6 max-w-[1440px] mx-auto w-full font-sans transition-colors duration-200 ${
    ...
    >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <button
          ...
          className="bg-emerald-700 hover:bg-emerald-800 text-white transition-all px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 shadow-md hover:shadow-lg cursor-pointer active:scale-95"
        >
      ...
      {/* Toolbar Area */}
      <div
        className={`flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 p-4 rounded-2xl border transition-colors ${
          isDark ? 'bg-[#18181b] border-[#27272a] shadow-md' : 'bg-white border-slate-200 shadow-xs'
        }`}
      >
        {/* Search Input */}
        <div className="relative flex-1 w-full md:max-w-md">
          ...
          <input
            ...
            className={`w-full border rounded-xl pl-9 pr-4 py-2 text-xs font-semibold transition-all ${
              isDark
                ? 'bg-[#27272a] border-[#3f3f46] text-zinc-100 placeholder:text-zinc-500 focus:bg-[#202024] focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500'
        ...
        <div className={`text-xs font-bold ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
          ...
          <span className="font-mono text-emerald-500 font-extrabold">{projects.length}</span>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProjects.map((project) => {
            ...
            return (
              <article
                key={project.id}
                className={`border rounded-2xl p-5 flex flex-col hover:shadow-xl transition-all group relative overflow-hidden ${
                  isDark
                    ? 'bg-[#18181b] border-[#27272a] hover:border-emerald-500/60 text-zinc-100 shadow-lg'
              ...
              >
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className={`p-1.5 rounded-lg transition-colors cursor-pointer ${
                        isDark
                          ? 'text-zinc-400 hover:text-emerald-400 hover:bg-[#27272a]'
                ...
                {/* Specs Box */}
                <div
                  className={`grid grid-cols-2 gap-3 my-3 py-3 border-y rounded-xl px-3 text-xs ${
                    isDark
                ...
                >
                  ...
                  <div>
                    ...
                    <span className={`font-bold flex items-center gap-1 ${isDark ? 'text-zinc-200' : 'text-slate-700'}`}>
                ...
                <div className="mt-auto pt-2">
                  <div className={`flex items-center justify-between text-[11px] mb-4 font-medium ${isDark ? 'text-zinc-500' : 'text-slate-400'}`}>
                    <span className="flex items-center gap-1">
                      <Calendar className="w-3.5 h-3.5" />
                      Guardado: {dateStr}
                    </span>
                    <span className={`font-semibold ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
                  ...
                  <button
                    ...
                    className="w-full py-2.5 bg-emerald-700 hover:bg-emerald-800 text-white font-bold text-xs rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-xs hover:shadow-md cursor-pointer active:scale-95"
                  >
