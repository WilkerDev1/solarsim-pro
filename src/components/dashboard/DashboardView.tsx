...
export const DashboardView: React.FC = () => {
  ...
  // Conjunto de IDs de carpetas configuradas para ocultarse de la vista general
  ...
  const filteredAndSortedProjects = useMemo(() => {
    const list = projects.filter((project) => {
      ...
      const province = (client.province || '').toLowerCase();
      const projectId = (client.projectId || '').toLowerCase();
      const quoteNumber = (client.quoteNumber || '').toLowerCase();
      const matchesSearch =
        !query ||
        name.includes(query) ||
      ...
      // 2. Folder Filter: si hay carpeta activa, mostrar sólo las de esa carpeta.
      // Si estamos en la vista general (sin carpeta seleccionada), ocultar los proyectos de carpetas con hideFromGeneral.
      const matchesFolder = activeFolderId
        ? project.folderId === activeFolderId
        : (!project.folderId || !hiddenFolderIds.has(project.folderId));
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {activeFolderName && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-extrabold text-emerald-700 dark:text-emerald-300 uppercase tracking-tight flex items-center gap-1.5 bg-emerald-50 dark:bg-emerald-950/50 px-3 py-1.5 rounded-xl border border-emerald-200 dark:border-emerald-800 shadow-2xs">
                  <Folder className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
                  <span>{activeFolderName}</span>
                  {activeFolder?.hideFromGeneral && (
                    <span title="Oculta del menú principal" className="flex items-center text-amber-500 dark:text-amber-400 ml-0.5">
                  ...
                  <button
                    ...
                    className="ml-1 text-emerald-600 hover:text-emerald-900 dark:hover:text-white cursor-pointer p-0.5 rounded-full hover:bg-emerald-100 dark:hover:bg-emerald-900 transition-colors"
                    title="Quitar filtro de carpeta y ver todos los proyectos"
                  >
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {activeFolderName && (
                <span className="px-2.5 py-1 rounded-lg bg-white dark:bg-[#181d27] border border-emerald-300 dark:border-emerald-700 font-bold text-emerald-800 dark:text-emerald-300 flex items-center gap-1.5">
                  ...
                  {activeFolder?.hideFromGeneral && (
                    <span title="Oculta del menú principal" className="flex items-center text-amber-500 dark:text-amber-400">
